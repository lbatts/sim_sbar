  ...
  #===============constant (c)
        ...
        #set and reset these stks up before applying fbars, sr residuals and catch error
           ...
           # plot(stk_rc,stk_ow)
            ...
            eps_ind <- list()
            eps_ind <- lapply(1:3,function(x){FLQuant(rlnorm(n = prod(dim(stk_c)), sdlog = 0.0001), dim = dim(stk_c), dimnames = dimnames(landings.n(stk_c)))
            })
...
#               
            res[3:5] <- lapply(res[3:5], catlowerror)
            # 
