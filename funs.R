...
assessments<- function(xx){
  ...
  for (j in 1:iters){
   ...
   # print("load firstschnu")
    ...
    suppressWarnings(
      if(is.null(schnualt)){
        xx$schnualt_par[1:2,1,j]<-"obj evals to NaN"  
      }else if(schnualt==1){
        xx$schnualt_par[1:2,1,j]<-"did not converge"
      }else if(schnualt=="try different starting pars"){
        xx$schnualt_par[1:2,1,j]<-"try different starting pars"
      }else{
    ...
    suppressWarnings(
      if(is.null(schnub0)){
        xx$schnub0_par[1:3,1,j]<-"obj evals to NaN"  
      }else if(schnub0==1){
        xx$schnub0_par[1:3,1,j]<-"did not converge"
      }else if(schnub0=="try different starting pars"){
        xx$schnub0_par[1:3,1,j]<-"try different starting pars"
      }else{
    ...
    }else xx$schnub0_par[1:3,1,j]<- xx$schnualt_par[1:2,1,j] <- "rho error"    #this is end of if to negate against logrho errors
    ...
    suppressWarnings(
      if(is.null(csaest)){
        xx$csa_par[1:3,1,j]<-"obj evals to NaN"  
      }else if(csaest==1){
        xx$csa_par[1:3,1,j]<-"did not converge"
      }else if(csaest=="try different starting pars"){
        xx$csa_par[1:3,1,j]<-"try different starting pars"
      }else{
...
#####==============assessment with sigma estimation
assessments_sigmaest<- function(xx){
  stk<-xx$stk
  pr_range <-2:(stk@range["max"]+1)
  f_range <- 1:(stk@range["max"]+1)
  no.years <-max(an(dimnames(stk)$year))
