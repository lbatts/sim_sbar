...
iters<- 100
timing<-0
seedlist <- seq(from = 100,by = 50,length.out = 96)
...
# 
# rm(list=ls())
# load("res_iter8.Rdata")
# #load("res_10iters_noassess.Rdata")
# source("funs.R")
# seedlist <- seq(from = 100,by = 50,length.out = 96)
# #test1<-res
# # test2<-tmpres
...
# # test1$scenario
# # test2[[8]]$scenario
# # test1$stk_c$mwts[,,10]-test2[[8]]$stk_c$mwts[,,10]
# #for(i in 1:96){
# #  print(paste("iterations",i))
# set.seed(seedlist[1])
jeff[[i]]<-lapply(res[3:5], assessments)
# 
...
#save(jeff,file="res1to10_notpar.RData")
...
test1[[1]]$stk_c$mwts[,,10]-test2[[1]]$stk_c$mwts[,,10]# not the same 

test1[[1]]$stk_c$stk@catch.n[3,7]
test2[[1]]$stk_c$stk@catch.n[3,7]


rec(iter(test1$stk_c$stk,7))- rec(iter(test2$stk_c$stk,7))

tmpres[[1]]$stk_c$schnualt_par[,,1]
tmpres[[10]]$stk_rc$schnualt_par[,,1]
